...
authors:  
  - name: David Hammond
    affiliation: Institute for Economics and Peace
    address:
    - 205 Pacific Highway
    - Sydney, Australia
...
knitr::kable(head(violence), format = "html", caption = "World Risk Poll Search Result Example") 
```
...
As shown in Table 2, the following lines of code return a dataframe with six columns:
...
```{r, message = FALSE, warning = FALSE, eval = knitr::is_html_output(), layout = "l-body-outset"}
knitr::kable(head(work_violence), format = "html", caption = "World Risk Poll Data Retrieval Example") 
...
-   `geography`: lists the selected geographic units

-   `disaggregation`: lists the disaggregation category

-   `group`: lists the different segments within the disaggregation category

-   `year`: lists the year the survey was administered

-   `question`: lists the full question name

-   `response`: lists the possible answers to each question

-   `percentage`: lists different groups' response rates to each possible answer
...
# Search for questions around violent crime
knitr::kable(head(violent_crime), format = "html", caption = "World Risk Poll Violent Crime Data Retrieval Example") 
```
